(function () {
	...
	function fromQueryToTags(query) {
		...
		angular.forEach(query, function(v) {
			var r = {};
			if(v.type !== 'FREETEXT') {
				r = {value: (v.type === 'USER_LABEL' ? '#' : '') + v.name + (v.value ? (':' + quoteIfHasSpace(v.value.originalValue || v.value.value)) : '')};
			}  else {
			...
			link: function ($scope, element, attributes) {
				...
				function searchUser(input, prefix, data) {
					return $http.get('api/search/user', {params: toParams(input, prefix)}).then(function (res) {
						for (var i = 0; i < res.data.length; i++) {
							var u = res.data[i];
							data.push({value: prefix + u.provider + ":" + u.username + " ", text: prefix + u.provider + ":" + u.username, type: "create", user: u, prefix: prefix});
						}
						return data
					});
				}
				...
				function searchMilestone(input, prefix, data) {
					return $http.get('api/search/milestone', {params: toParams(input, prefix)}).then(function (res) {
						for (var i = 0; i < res.data.length; i++) {
							var u = res.data[i];
							data.push({value: prefix + quoteIfHasSpace(u), text: prefix + quoteIfHasSpace(u), type: "create"});
						}
						return data;
					});
				}
				...
				$scope.configuration = {
						...
						autocompleteProvider:function(input) {
							...
							if (input.indexOf("to:") === 0) {
								...
								res.push({value: "updated:previous month ", text: "updated:previous month", type: "create"});
								...
								res.push({value: "status:DEFERRED ", text: "status:DEFERRED", type: "create"});
							  	...
							  	res.push({value: "created:previous month ", text: "created:previous month", type: "create"});
				...
				/*var resizeSearchBar = function () {
					$(".lavagna-search").css('width', $window.innerWidth < 1024 ? '500px' : '650px');
				};
